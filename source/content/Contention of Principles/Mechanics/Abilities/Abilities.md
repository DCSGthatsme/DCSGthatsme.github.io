...
Novice<br>Passive. In the character's hands, a jointed weapon gains the quality Ensnaring, and may therefore be used to ensnare an enemy, instead of making a normal attack.
...
Master<br>Active. The master Medic has a vast medical knowledge. A successful Wit test now heals 1D8 Toughness, or 1D10 when using an herbal cure. Even a failed test heals some of the patient's Toughness; 1D4 when only using Medicus or 1D6 when also applying an herbal cure. This ability can only be used once per patient and once per day.
...
Novice<br>Reaction. The character is an experienced rider and is accustomed to fighting while mounted. The character knows how to leverage the weight of the mount in a cavalry charge and deals 1D6 additional damage with one melee attack if the mount moves prior to the attack.
